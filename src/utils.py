...
def filter_transactions(list_transactions: list, date_to: str) -> DataFrame:
    ...
    try:
        ...
        df_date = df_transactions[
            (df_transactions["Дата операции"].dt.day <= date_day)
            & (df_transactions["Дата операции"].dt.month == date_month)
            & (df_transactions["Дата операции"].dt.year == date_year)
        ]
...
def read_json_from_file(path_to_file: str) -> tuple[list, list]:
...
def get_stocks_prices(stocks: list) -> list:
    ...
    for _ in range(len(stocks)):
        ...
        price_stock.append({"stock": data_stock["ticker"], "price": data_stock["price"]})
...
def get_exchange_currency(currencies: list) -> list:
    ...
    get_params = {"get": "rates", "pairs": pairs, "key": apy_key}
    ...
    current_currencies = [
        {"currency": currency_pair[:3], "rate": data_currency["data"][currency_pair]}
